// Défini des elements du DOMfalse

const   SectionPanierItem = document.getElementById("cart__items")
const   SectionPanier = document.getElementById("cart")
...
const retrieveApiData = async () => fetch(`http://localhost:3000/api/products/${ItemInfo.id}`)
  .then(res => res.json())
  .then(data => data)
...
const createCartItemImg = cartItem => {
  ...
  const   img = document.createElement("img")
    img.setAttribute("src", ItemInfoApi.imageUrl)
    img.setAttribute("alt", ItemInfoApi.altTxt)
...
function createCartItemContentDescription() {
  ...
  const   itemName = document.createElement("h2")
    itemName.textContent = ItemInfoApi.name

  const   itemColor = document.createElement("p")
    itemColor.textContent = ItemInfo.color
  const   itemPrice = document.createElement("p")
    itemPrice.textContent = ItemInfoApi.price + " €"
...
const createCartItemContentSettingsQuantity = cartItem => {
  ...
  const   itemQuantity = document.createElement("input")
   ...
   itemQuantity.setAttribute("value", ItemInfo.quantity)
...
// Calculer prix total en modifiant la variable price 
...
const cartPrice = () => {
  ...
  if (Panier.Item.length === 0) {
    ...
    for (let numberOfArticles = 0; numberOfArticles <= Panier.Item.length -1 ; numberOfArticles++) {
      let numberInCart = +Panier.Item[numberOfArticles].quantity
      number = number + numberInCart
      totalQuantity.textContent = number

      price = number * ItemInfoApi.price
    }
  }
...
const quantityInputsChange = async (e) => {
  ...
  let itemIndex = Panier.Item.findIndex(item => item.id === targetId && item.color === targetColor)

  if (Panier.Item[`${itemIndex}`].quantity < e.target.value) {
    price =  price + ItemInfoApi.price
  } else  {
    price = price - ItemInfoApi.price
  }

  Panier.Item[`${itemIndex}`].quantity = e.target.value

  localStorage.setItem("cartItems", JSON.stringify(SectionPanier))

  number = 0

  ItemInfo.id = targetId

  ItemInfoApi = await retrieveApiData()
...
const deleteItem = async (e) => {
  ...
  console.log(targetQuantPrice);
   ...
   SectionPanierItem.removeChild(targetItem)
  Panier.Item.splice(itemIndex, 1,)
...
function createOrder(event) {
  ...
  if (formVerification === true) {
    ...
    for (let productsN = 0; productsN < Panier.Item.length -1; productsN++) {
      products.push(Panier.Item[`${productsN}`].id)
    }
...
const main = async () => {
  Panier = JSON.parse(localStorage.getItem("ItemPanier"))

  if (Panier !== null) {
    const numberOfCartItems = Panier.Item.length - 1

    for (let i = 0; i <= numberOfCartItems; i++) {
      ItemInfo = Panier.Item[i]

      ItemInfoApi = await retrieveApiData()

      quantPrice = ItemInfoApi.price * ItemInfo.quantity

      console.log(quantPrice);


       SectionPanierItem.appendChild(createCartItemArticle(ItemInfo))
